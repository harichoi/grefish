...
BOOL CGrefishmapDlg::OnInitDialog()
{
	...
	{
		sprintf(str,"%d.bmp",i);
		aaa=(HBITMAP)LoadImage(AfxGetInstanceHandle(),str, IMAGE_BITMAP,  0, 0,  LR_LOADFROMFILE);
		blockbit[i]=aaa;//(CBitmap)CBitmap::FromHandle(aaa);
	}
	...
	blockbit[DOWNUP]=LoadBitmap(AfxGetInstanceHandle(),MAKEINTRESOURCE(IDB_DOWNUP));
...
void CGrefishmapDlg::OnPaint() 
{
	...
	TextOut(MemDC,x-CSX-ggx,y-CSY-ggy,"○",2);*/
		...
		if(bgx==gx && bgy==gy)
		if(x-CSX-ggx>0 && y-CSY-ggy>0)
		if(x-CSX-ggx<18*30 && y-CSY-ggy<30*16)
...
//	a.prints();
	...
	a.DoModal();
	effect[num]=a.eft;
	effect[num].evx=a.m_svx;
	effect[num].evy=a.m_svy;
...
void CGrefishmapDlg::OnButton14() 
{
	...
	a.m_svx=effect[m_number].evx;
	a.m_svy=effect[m_number].evy;
	a.eft=effect[m_number];
	a.number=m_number;
//	a.prints();
	a.DoModal();
	effect[m_number]=a.eft;
	if(a.eft.exist==0)
	{
		for(i=1 ; i<=efn[gy/16+1][gx/18+1].enef ; i++)
			if(efn[gy/16+1][gx/18+1].eef[i]==m_number)
	...
	{
		effect[m_number].evx=a.m_svx;
		effect[m_number].evy=a.m_svy;
